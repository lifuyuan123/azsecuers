...
public class SoftwareManager {
	...
	/** 用来保存所有应用实体的HashMap */
	private HashMap<Integer, List<Softinfo>> softwareInfos;
	...
	private SoftwareManager(Context context) {
		...
		// 将用来保存应用程序的三个集合
		softwareInfos = new HashMap<Integer, List<Softinfo>>();
		// 将用来保存 所有应用程序
		softwareInfos.put(CLASSIFY_ALL, new ArrayList<Softinfo>());
		// 将用来保存 系统应用程序
		softwareInfos.put(CLASSIFY_SYS, new ArrayList<Softinfo>());
		// 将用来保存 用户应用程序
		softwareInfos.put(CLASSIFY_USER, new ArrayList<Softinfo>());
	}

	/** 获取手机上指定分类的应用实体数据集合 */
	public List<Softinfo> getSoftware(int classify) {
		// 清空内部所有数据
	...
	private void loadSoftwareInfos() {
		...
		for (ResolveInfo resolveInfo : resolveInfos) {
			...
			// 分类整合数据
			Softinfo info = new Softinfo(lable, packagename, version, drawable);
			int flags = resolveInfo.activityInfo.applicationInfo.flags;
			classifySoftware(info, flags);
		}
	}

	private void classifySoftware(Softinfo info, int flags) {
	...
	@Deprecated
	public List<Softinfo> getAllSoftware() {
		List<Softinfo> browseInfos = new ArrayList<Softinfo>();
		...
		for (PackageInfo packageInfo : packageInfos) {
			...
			// 重新实例为我们所需的实体,保存到集合内
			Softinfo browseInfo = new Softinfo(lable, packagename, version, drawable);
